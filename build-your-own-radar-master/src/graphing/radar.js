...
const { uploadCSVFile } = require('../../spec/end_to_end_tests/pageObjects/radar_page')
...
const Radar = function (size, radar) {
  ...
  function plotQuadrant (rings, quadrant) {
    ...
    rings.forEach(function (ring, i) {
      var arc = d3.arc()
        ...
        .outerRadius(ringCalculator.getRadius(i + 2))
    ...
    rings.forEach(function (ring, i) {
      if (quadrant.order === 'first' || quadrant.order === 'fourth') {
        quadrantGroup.append('text')
          ...
          .attr('y', center() + 1)
          .attr('x', center() +(75 + (ringCalculator.getRadius(i)) + ringCalculator.getRadius(i)) / 2)
  ...
  function calculateBlipCoordinates (blip, chance, minRadius, maxRadius, startAngle) {
    ...
    var radius = chance.floating({ min: minRadius + blip.width / 3, max: maxRadius - blip.width / 3 })
    var angleDelta = Math.asin(blip.width / 3 / radius) * 180 / Math.PI
  ...
  function plotRadarHeader () {
    ...
    header.select('.radar-title')
      ...
      .html('<a href="https://www.hms.com"> <img src="/images/logo.png" /> </a>')
  ...
  function plotQuadrantButtons (quadrants, header) {
   ...
   /*
    alternativeDiv.append('div')
      ...
      .attr('placeholder', 'Select CSV File')*/
  ...
  function plotRadarFooter () {
    d3.select('body')
      ...
      .append('p')
      .html('Powered by <a href="https://www.hms.com"> HMS Corp INC</a>. ' +
      'By using this service you agree to <a href="https://www.hms.com">HMS\' terms of use</a>. ' +
      'You also agree to our <a href="https://www.hms.com">privacy policy</a>, which describes how we will gather, use and protect any personal data contained in your public Google Sheet. ' +
      'This software is <a href="Links Has to be uploaded">open source</a> and available for download and self-hosting.')
  }
